...
function rainGrowthFactor(rainAmount = 0) {
	const clampedRain = Math.max(0, Math.min(globalThis.SIM.weather.darkRainMaxMm, rainAmount));
	return Math.pow(2, 0.00043 * clampedRain) - 1; // y=(2^0.00043x)-1
}
...
function rainAffectsTerrain(x, y, rainAmount = 0) {
	...
	if (raw <= DARK_GRASS_MAX || rainAmount <= 0) return;

	const delta = rainGrowthFactor(rainAmount);
	if (delta <= 0) return;
